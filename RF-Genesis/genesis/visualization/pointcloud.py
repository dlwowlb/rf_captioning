...
class FrameConfig: #
    def __init__(self, cfg:Radar):
        ...
        self.doppler_resolution = cfg.doppler_resolution

class PointCloudProcessCFG: #
    def __init__(self, radar:Radar):
        self.frameConfig = FrameConfig(radar)
        ...
        self.sumCouplingSignatureArray = np.zeros((self.frameConfig.numTxAntennas,self.frameConfig.numRxAntennas,self.couplingSignatureBinFrontIdx+self.couplingSignatureBinRearIdx),dtype = complex)
...
def frame2pointcloud(frame,pointCloudProcessCFG):
    ...
    if pointCloudProcessCFG.outputInMeter:
        R *= pointCloudProcessCFG.frameConfig.range_resolution
        V *= pointCloudProcessCFG.frameConfig.doppler_resolution
...
#         reshaped = frameReshape(np_frame,pointCloudProcessCFG.frameConfig)
#         pointCloud = frame2pointcloud(reshaped,pointCloudProcessCFG)
