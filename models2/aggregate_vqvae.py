...
class MaskedContextAggregation(nn.Module):
    def __init__(self, feat_dim=512, num_anchors=64, num_layers=4,
                 num_heads=8, mask_ratio=0.5, dropout=0.3):
        super().__init__()
...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, z_e):
        ...
        # ====================================================
        # ★ FIX: Dead Code Revival (죽은 코드북 벡터 강제 활성화)
        # ====================================================
        if self.training:
            # 현재 미니배치에서 선택된 토큰의 고유 인덱스 추출
            unique_indices = torch.unique(indices)
            
            # 선택되지 않은(Dead) 토큰 인덱스 찾기
            all_indices = torch.arange(self.codebook_size, device=z_e.device)
            dead_mask = ~torch.isin(all_indices, unique_indices)
            dead_indices = all_indices[dead_mask]

            # 죽은 토큰이 있다면, 현재 배치의 z_e 값 중 무작위로 가져와서 덮어쓰기
            if len(dead_indices) > 0:
                rand_idx = torch.randint(0, flat.shape[0], (len(dead_indices),), device=z_e.device)
                with torch.no_grad():
                    self.codebook.weight[dead_indices] = flat[rand_idx].detach()
                
                # 가중치가 변경되었으므로 거리와 인덱스를 다시 계산하여 정확성 유지
                dist = (flat.pow(2).sum(1, keepdim=True)
                        + self.codebook.weight.pow(2).sum(1, keepdim=True).t()
                        - 2 * flat @ self.codebook.weight.t())
                indices = dist.argmin(dim=1)
