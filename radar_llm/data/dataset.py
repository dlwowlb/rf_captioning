...
Generated by generate_data.py via RF-Genesis simulation pipeline:
  Text → HY-Motion → SMPL → Ray Tracing → Radar Signal → Point Cloud

Point cloud format from RF-Genesis: (T, N, 6) = [x, y, z, velocity, SNR, range]
The dataset selects point_dim channels (default 5: x,y,z,velocity,SNR) and
applies per-sample normalization for stable training.

Expected data directory structure:
    data_root/
    ├── radar/                      # Radar point cloud sequences
    │   ├── sample_0000_aug00.npy   # (T, N, 6) arrays from RF-Genesis
    │   └── ...
    ├── motion/                     # SMPL motion parameters
    │   ├── sample_0000_aug00.npz   # {pose: (T, 72), shape: (10,), root_translation: (T, 3)}
    │   └── ...
    ├── captions/                   # Text captions (one per sample)
    │   ├── sample_0000_aug00.txt
    │   └── ...
    ├── splits/
    │   ├── train.txt
    │   └── val.txt
...
class RadarCaptionDataset(Dataset):
    ...
    def _normalize_point_cloud(self, data: np.ndarray) -> np.ndarray:
        """Normalize point cloud data per sample.

        For RF-Genesis data (x,y,z,velocity,SNR,...):
        - xyz: center to mean, scale to unit sphere
        - velocity: standardize (zero-mean, unit-var)
        - SNR/other: min-max normalize to [0, 1]
        """
        ...
        data[:, :, :3] -= center
        scale = np.linalg.norm(data[:, :, :3].reshape(-1, 3), axis=1).max()
        if scale > 1e-6:
            data[:, :, :3] /= scale

        # velocity channel (3) - standardize
        if data.shape[-1] > 3:
            v = data[:, :, 3].reshape(-1)
            v_std = v.std()
            if v_std > 1e-6:
                data[:, :, 3] = (data[:, :, 3] - v.mean()) / v_std

        # remaining channels - min-max to [0, 1]
        for d in range(4, data.shape[-1]):
            ch = data[:, :, d].reshape(-1)
            ch_min, ch_max = ch.min(), ch.max()
            if ch_max - ch_min > 1e-6:
                data[:, :, d] = (data[:, :, d] - ch_min) / (ch_max - ch_min)
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if os.path.exists(radar_path):
            ...
            # Normalize per-sample for stable training
            if self.normalize:
                radar_data = self._normalize_point_cloud(radar_data)
        else:
